...
magma_int_t matInv_magma(double *A, magma_int_t n)
{
	...
	magma_int_t *ipiv = new magma_int_t [N];
        magma_int_t ret,ret1;
        
        double *dwork = new double [ldwork];

        
        magma_dgetrf_gpu(N,N,A,ldda,ipiv,&ret);
...
cout << "value of n "<<n<<endl;
        magma_dgetri_gpu(n, A, n, ipiv, dwork, ldwork, &ret1);
cout << "value of ret after "<<ret1<<endl;
...
for (int i=0; i<n*n; i++) {
    ...
    ret = LAPACKE_dgetri(LAPACK_COL_MAJOR,
                       ...
                       n,
                       ipiv);
 cout << endl<<"Matrix A inside lapack _ inverse dgetri"<<endl;
for (int i=0; i<n*n; i++) {
        if ((i%n) == 0) putchar('\n');
        printf("%+12.8f ",A[i]);
    }
