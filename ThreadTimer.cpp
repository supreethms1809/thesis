...
LONGLONG ThreadTimer::DisplayFrequency(void)
{
	static LARGE_INTEGER performance_freq;
	static bool qpf_called = false;
...
double** ThreadTimer::SetupElapsedTimeMatrix(int nrows, int ncols)
{
	double **et = new double *[nrows];
	for (int i = 0;i < nrows;i++)
	{
		et[i] = new double[ncols];
	}
	return et;
}